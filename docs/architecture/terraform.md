...
**Framework Design**:
- **Language**: Pure Bash + jq (no external dependencies)
- **Execution**: Parallel test execution for performance
- **Coverage**: 269 individual assertions across all modules
...
|--------|------------|-------------|
| **S3 Module** | 49 tests | Security, replication, lifecycle, compliance |
| **CloudFront Module** | 55 tests | OAC, security headers, caching, performance |
| **WAF Module** | 50 tests | Rule sets, rate limiting, geo-blocking, logging |
| **IAM Configuration** | 58 tests | OIDC, trust policies, permissions, security |
| **Monitoring Module** | 57 tests | Dashboards, alarms, notifications, budgets |
...
flowchart TD
    %% Accessibility
    accTitle: Terraform Testing Execution Flow
    accDescr: Shows testing workflow from test runner through module validation to reporting. Process includes environment setup, parallel test execution across all modules, assertion validation, and comprehensive JSON reporting for CI/CD integration.
    
    A[Test Runner] --> B[Environment Setup]
    B --> C[Parallel Execution]
    
    C --> D1[S3 Tests]
    C --> D2[CloudFront Tests] 
    C --> D3[WAF Tests]
    C --> D4[IAM Tests]
    C --> D5[Monitoring Tests]
    
    D1 --> E[Assertion Validation]
    D2 --> E
    D3 --> E
    D4 --> E
    D5 --> E
    ...
    %% High-Contrast Styling for Accessibility
    classDef runnerBox fill:#fff3cd,stroke:#856404,stroke-width:3px,color:#212529
    ...
    class A,B,C runnerBox
    class D1,D2,D3,D4,D5 testBox
    class E,F validationBox
    class G,H reportBox
```
